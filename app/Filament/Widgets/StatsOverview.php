...
class StatsOverview extends BaseWidget
{
    ...
    protected function getStats(): array
    {
        ...
        $transaction = Transaction::whereStatus('approved')
            ...
            ->whereYear('created_at', Carbon::now()->year);
        $transactionLastMonth = Transaction::whereStatus('approved')
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)
            ->whereYear('created_at', Carbon::now()->subMonth()->year);
        $transactionPercentage = $this->getPercentage(
            $transactionLastMonth->count(),
            $transaction->count()
        );
        $revenuePercentage = $this->getPercentage(
            $transactionLastMonth->sum('total_price'),
            ...
            Stat::make('New Transaction of the month', $transaction->count())
                ->description(
                    $transactionPercentage > 0
                        ? "{$transactionPercentage}% increased"
                        : "{$transactionPercentage}% decreased"
                )
                ->descriptionIcon(
                    $transactionPercentage > 0
                ...
                ->color($transactionPercentage > 0 ? 'succes' : 'danger'),
            Stat::make(
                'New revenue of the month',
                Number::currency($transaction->sum('total_price'), 'USD')
            )
                ->description(
                    $revenuePercentage > 0
                        ? "{$revenuePercentage}% increased"
                        : "{$revenuePercentage}% decreased"
                )
                ->descriptionIcon(
                    $revenuePercentage > 0
                        ? 'heroicon-m-arrow-trending-up'
                ...
                ->color($revenuePercentage > 0 ? 'succes' : 'danger'),
        ];
    }
}
